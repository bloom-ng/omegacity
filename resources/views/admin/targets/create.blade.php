    ...
    <div class="w-full">
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-gray-600">Set monthly and yearly targets for revenue, sales, and leads</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="p-8">
                <form action="{{ route("admin.targets.store", $agent) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="targets[0][period_type]" id="period_type" required
                                ...
                                <option value="monthly" {{ old("targets.0.period_type") === "monthly" ? "selected" : "" }}>
                                    Monthly Target</option>
                                <option value="yearly" {{ old("targets.0.period_type") === "yearly" ? "selected" : "" }}>
                                    Yearly Target</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="targets[0][target_type]" id="target_type" required
                                ...
                                <option value="amount" {{ old("targets.0.target_type") === "amount" ? "selected" : "" }}>
                                    Revenue Amount (₦)
                                </option>
                                <option value="sales" {{ old("targets.0.target_type") === "sales" ? "selected" : "" }}>
                                    Sales Count
                                </option>
                                <option value="leads" {{ old("targets.0.target_type") === "leads" ? "selected" : "" }}>
                                    Leads Count
                                </option>
                        ...
                        <div>
                            ...
                            <select name="targets[0][month]" id="month"
                                ...
                                @for ($month = $currentMonth; $month <= 12; $month++)
                                    <option value="{{ $month }}"
                                        {{ old("targets.0.month") == $month ? "selected" : "" }}>
                        ...
                        <div>
                            ...
                            <select name="targets[0][year]" id="yearly_year"
                                ...
                                @for ($year = $currentYear; $year <= $currentYear + 5; $year++)
                                    <option value="{{ $year }}"
                                        {{ old("targets.0.year", $currentYear) == $year ? "selected" : "" }}>
                                        {{ $year }}
                                    </option>
                                @endfor
                            </select>
                    ...
                    <div id="existing_target_warning" class="bg-yellow-50 border border-yellow-200 rounded-md p-4"
                        style="display: none;">
                        <div class="flex">
        ...
        <script>
            ...
            function updateTargetHint() {
                ...
                if (targetType === 'amount') {
                    ...
                    targetHint.textContent = `Enter the ${period} sales count target (e.g., 10 sales)`;
                    targetInput.step = '1';
                    targetInput.min = '1';
                } else if (targetType === 'leads') {
                    targetHint.textContent = `Enter the ${period} leads target (e.g., 50 leads)`;
                    targetInput.step = '1';
                    targetInput.min = '1';
                }
            }
            ...
            function checkExistingTarget() {
                ...
                if (existing) {
                    ...
                    if (targetType === 'amount') {
                        typeDisplay = 'revenue';
                        valueDisplay = `₦${Number(existing.target_value).toLocaleString()}`;
                    } else if (targetType === 'sales') {
                        typeDisplay = 'sales';
                        valueDisplay = `${existing.target_value} sales`;
                    } else if (targetType === 'leads') {
                        typeDisplay = 'leads';
                        valueDisplay = `${existing.target_value} leads`;
                    }

                    warningMessage.textContent =
                        `A ${periodType} ${typeDisplay} target already exists for ${periodDisplay}: ${valueDisplay}. Creating a new target will update the existing one.`;
