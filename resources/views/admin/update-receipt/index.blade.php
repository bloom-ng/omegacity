...
@section("content")
    <div class="w-full">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Updated Receipts</h1>
            <a href="{{ route("admin.update-receipts.create") }}"
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div class="flex items-center space-x-2">
                        <form method="GET" action="{{ route("admin.update-receipts.index") }}" class="relative max-w-xs w-full">
                        ...
                        @if (request("search"))
                            <a href="{{ route("admin.update-receipts.index") }}" class="text-sm text-blue-600 hover:underline">
                                Clear
                            </a>
                        @endif
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($update_receipts as $receipt)
                            @php
                                $items = $receipt->receipt_items ?? [];
                                $totalQuantity = collect($items)->sum(fn($item) => (int) ($item['quantity'] ?? 1));
                            @endphp
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route("admin.update-receipts.show", $receipt->id) }}"
                                        class="text-blue-600 hover:text-blue-900 mr-2"><i class="fas fa-eye"></i></a>

                                    <a href="{{ route("admin.update-receipts.edit", $receipt->id) }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-2"><i class="fas fa-edit"></i></a>

                                    <form action="{{ route("admin.update-receipts.destroy", $receipt->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method("DELETE")
                                    ...
                                    <a href="{{ route("admin.update-receipts.pdf", $receipt) }}"
