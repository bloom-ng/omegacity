        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 space-y-3 sm:space-y-0">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Receipt:
                    REC-{{ $update_receipt->created_at->format("Ymd") }}{{ $update_receipt->id }}</h1>
                <div class="flex items-center">

                    <div class="ml-4 text-sm text-gray-500">
                        Created on {{ $update_receipt->created_at->format("M d, Y") }}
                    </div>
                </div>
            </div>
            <div class="flex space-x-2">
                ...
                <a href="{{ route("admin.receipts.edit", $update_receipt) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mt-1 text-sm font-bold text-gray-900">
                            {{ $update_receipt->client->first_name }} {{ $update_receipt->client->last_name }}<br>
                            {{ $update_receipt->client->address }}<br>
                            {{ $update_receipt->client->email }}<br>
                            {{ $update_receipt->client->phone }}
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span
                                class="text-sm text-gray-900 font-bold">REC-{{ $update_receipt->created_at->format("Ymd") }}{{ $update_receipt->id }}</span>
                            <span class="text-sm text-gray-900 font-bold">{{ $update_receipt->date->format("M d, Y") }}</span>
                        </div>
                        ...
                        <div class="flex justify-between">
                            <span class="text-sm text-gray-900 font-bold">{{ $update_receipt->commission_percentage }}%</span>
                            <span
                                class="text-sm text-gray-900 font-bold">₦{{ number_format($update_receipt->commission_amount) }}</span>
                        </div>
            ...
            <div class="px-4 py-5 sm:p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mt-1 text-sm font-bold text-gray-900">
                            Payment Type: {{ str_replace("_", " ", $update_receipt->payment_type) }} <br>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
                <div class="flex justify-end">
                    <div class="w-64">
                        ...
                        @if ($update_receipt->tax > 0)
                            <div class="flex justify-between py-2 text-sm text-gray-600">
                                <span>VAT ({{ $update_receipt->tax }}%)</span>
                                <span>₦{{ number_format($taxAmount) }}</span>
                            </div>
                        @endif

                        @if ($update_receipt->discount > 0)
                            <div class="flex justify-between py-2 text-sm text-gray-600">
                                <span>Discount ({{ $update_receipt->discount }})%</span>
                        ...
                        <div
                            ...
                            <span>Total</span>
                            <span>₦{{ number_format($update_receipt->grand_total, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
