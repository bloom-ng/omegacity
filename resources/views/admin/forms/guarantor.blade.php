...
@section("content")
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <form method="GET" action="{{ route("admin.forms.guarantor") }}" class="relative max-w-xs w-full">
                        <input type="text" name="search" value="{{ request("search") }}"
                            class="form-input w-full pl-10 pr-4 py-2 border rounded-lg" placeholder="Search guarantors...">
                        ...
                        @forelse($forms as $form)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <a href="{{ route("admin.forms.download", [$form->id, "guarantor"]) }}" target="_blank"
                                            ...
                                            <ul class="py-2 text-sm text-gray-700">
                                                ...
                                                <li class="px-4 py-2 hover:bg-gray-100">
                                                    @if ($form->document_file)
                                                        <a href="{{ route("admin.guarantor.download.file", ["id" => $form->id]) }}"
                                                            target="_blank" rel="noopener noreferrer"
                                                            class="flex items-center space-x-2">
                                                            <i class="fas fa-id-card"></i>
                                                            <span>Download File Document</span>
                                                        </a>
                                                    @else
    ...
    <div id="loading-overlay" class="fixed inset-0 bg-gray-100 bg-opacity-75 flex items-center justify-center z-50 hidden">
        <img src="{{ asset("assets/images/favicon-omega.png") }}" alt="Loading..." class="h-12 w-12 animate-spin">
    </div>
